<template>
	...
	>
		...
		<Suggestion
			...
			message="You are not logged in yet. Please use the messenger.com window below to sign in to your Facebook account."
...
export default {
	...
	mounted() {
		...
		webview.addEventListener("dom-ready", () => {
			...
			webview.addEventListener("console-message", e => {
				...
				let log = this.isStickyChatsLog(e.message);
				if(!log) return;
				if (log.messageType === 'updateUserStatus') {
					this.updateUserStatus(log.value);
				} else if (log.messageType === 'updateChats') {
					this.updateChats(log.value);
				}
			});
		});
			...
			webview.executeJavaScript(`
				if(document.querySelector('[data-href="https://www.messenger.com/new"]')) {
					console.log(JSON.stringify({
						isStickyChatsLog: true,
						messageType: 'userLoginStatus',
						value: 'loggedIn'
					}));
				}
				else {
	...
	methods: {
		isStickyChatsLog(message) {
			if (!message) return;
			try {
				let parsedJSON = JSON.parse(message);
				if (parsedJSON 
					&& typeof parsedJSON === "object"
					&& parsedJSON.hasOwnProperty('isStickyChatsLog')
					&& parsedJSON.isStickyChatsLog === true) {
					return parsedJSON;
				}
			} catch (e) {}
			return false;
